...
class Connection extends EventEmitter {
  ...
  _receive (data) {
    while (data.byteLength) {
      let frameSize = 4 + data.readInt32BE(0)
      let frameType = data.readInt32BE(4)
      let frameData = data.slice(8, frameSize)

      this._processFrame(frameSize, frameType, frameData)

      data = data.slice(frameSize)
    }
  ...
  async _pulse () {
    ...
    this._pulsing = (async () => {
      if (this.state === Pulsing || this.waiting || !this.queue.size) {
        return
      }

      if (this.state === Connecting || this.state === Identifying) {
        this.waiting = true
      ...
      for (const envelope of this.queue.values()) {
